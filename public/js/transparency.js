if ( Browser.Engine.trident4 ) { // Only do any of this in IE6

/*
Aurora Transparency Library, including slightly modified Aurora.Transparency
Copyright Jaze Technology 2011 and Drew Diller (see below)
*/

/**
* Aurora.Transparency: Adds IE6 support: PNG images for CSS background-image and HTML <IMG/>.
* Author: Drew Diller
* Email: drew.diller@gmail.com
* URL: http://www.dillerdesign.com/experiment/Aurora.Transparency/
* Version: 0.0.7a
* Licensed under the MIT License: http://dillerdesign.com/experiment/Aurora.Transparency/#license
*
* Example usage:
* Aurora.Transparency.fix('.png_bg'); // argument is a CSS selector
* Aurora.Transparency.fixPng( someNode ); // argument is an HTMLDomElement
**/

Aurora.Transparency = {

	ns: 'Aurora_Transparency',
	imgSize: {},
	...
	readPropertyChange: function() {
		...
		if (event.propertyName.search('background') != -1 || event.propertyName.search('border') != -1) {
			Aurora.Transparency.applyVML(el);
		}
		...
		if (event.propertyName.search('filter') != -1) {
			Aurora.Transparency.vmlOpacity(el);
		}
	},
	...
	**/
	fix: function(selector) {
		...
		for (var i=0; i<selectors.length; i++) {
			this.styleSheet.addRule(selectors[i], 'behavior:expression(Aurora.Transparency.fixPng(this))'); /* seems to execute the function without adding it to the stylesheet - interesting... */
		}
	},
...
Aurora.Transparency.createVmlNameSpace();
Aurora.Transparency.createVmlStyleSheet();

/* Auto-attach VML Replacement Method and Original Aurora Transparncy */
window.addEvent( 'domready', function() {
	
	Aurora.Transparency.fix('.has-transparency');
	
	$$('img.aurora-transparent-png').each(function(i){
		i.setStyles({
			filter: "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + i.get('src') + "',sizingMethod='image')",
			zoom: 1 // enforce hasLayout or the filter won't be applied
		});
		i.set( 'src', '/Platforms/Aurora/Common/Transparent.gif' );
	});
	
});

} // close global IF for IE6